...
export const saveTournamentResultsInFirestore = async (results: TournamentResult): Promise<number> => {
    ...
    // Create chunks of 450 to avoid Firestore limits
    const chunks = [];
...
export const clearTournamentResultsInFirestore = async (tournId: string, year: number | string): Promise<{ deletedCount: number }> => {
    ...
    await collectDocs('FantasyGolf-Results', numericYear);
    await collectDocs('FantasyGolf-Results', stringYear);
    ...
    console.log(`Cleared ${deletedCount} records for tournament ${tournId}.`);
    return { deletedCount };
};
