...
/**
 * Converts any date value from the API into a Firestore Timestamp.
 * Handles: ISO strings, numeric ms, numeric seconds, numeric strings, and MongoDB $date wrappers.
 */
const toTimestamp = (val: unknown): Timestamp | null => {
    if (val === null || val === undefined) return null;

    // Object — could be { $date: "..." }, { $date: { $numberLong: "..." } }, or a Timestamp
    if (typeof val === 'object') {
        const obj = val as Record<string, unknown>;
        // MongoDB Extended JSON date wrapper
    ...
    // Number — treat > 9999999999 as milliseconds, otherwise as seconds
    ...
    if (typeof val === 'string') {
        if (!val) return null;
        // Pure numeric string — treat as milliseconds
        if (/^\d+$/.test(val)) {
...
/**
 * Fetches the PGA Tour schedule from RapidAPI and saves it to Firestore.
 * Stores only the fields needed by the app. Uses tournId as the document ID
 * so re-syncing cleanly overwrites existing entries.
 */
export const syncPgaSchedule = async (year: string): Promise<number> => {
    ...
    // Delete all existing documents for this year before writing new ones
    // (removes stale docs with old auto-generated IDs)
    ...
    // Log the first raw item so we can verify the API response structure
