...
// A helper function to safely parse numeric values that might be objects from the database.
...
const RankingsPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setCurrentRankings(filteredRankings.slice(startIndex, endIndex));
  ...
  const getSortIcon = (key: keyof GolfRanking) => {
    if (!sortConfig || sortConfig.key !== key) {
      return <ChevronDown className="h-4 w-4 ml-1 opacity-25" />;
    }
        ...
        {pageNumbers.map((num, index) => (
          typeof num === 'number' ? (
            <button
              ...
              className={`pagination-button ${currentPage === num ? 'pagination-active' : ''}`}>
              {num}
            </button>
          ) : (
  ...
  if (error) {
    return <div className="text-red-500 text-center p-8">Failed to load rankings: {(error as Error).message}</div>;
  }
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      {loading ? (
        ...
        <div className="text-center p-8 bg-gray-50 rounded-lg shadow-inner">
          ...
          <p className="text-gray-500 mt-2">No rankings were found for the selected year. You may need to sync the data from the Admin page.</p>
        </div>
      ) : (
