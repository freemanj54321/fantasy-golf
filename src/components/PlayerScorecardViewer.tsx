...
import { ScorecardRound, HoleScore, savePlayerScorecard } from '../services/rapidApiService';
...
const PlayerScorecardViewer: React.FC<PlayerScorecardViewerProps> = ({
    ...
    const fetchScorecards = async () => {
        ...
        try {
            ...
            // If empty, fallback to string year (RapidAPI returns string years)
            if (snapshot.empty) {
                q = query(
                    scorecardsRef,
                    where("tournId", "==", tournId),
            ...
            const fetchedRounds: ScorecardRound[] = [];

            snapshot.forEach(doc => {
                fetchedRounds.push(doc.data() as ScorecardRound);
            });
            ...
            // Set active round to the latest round available by default
            if (fetchedRounds.length > 0) {
                setActiveRoundIndex(fetchedRounds.length - 1);
            }
    ...
    const handleSyncScorecard = async () => {
        ...
        setError(null);
        try {
            await savePlayerScorecard(tournId, year.toString(), playerId);
            await fetchScorecards(); // Refresh the UI after sync
        } catch (err) {
            console.error("Error syncing scorecard data:", err);
            setError("Failed to sync new scorecard data. Check logs.");
        } finally {
            setSyncing(false);
        }
    };
    ...
    const getScoreColor = (score: number, par: number) => {
        ...
        if (diff < -1) return 'bg-yellow-100 text-yellow-800 font-bold border-yellow-300'; // Eagle or better
        if (diff === -1) return 'bg-red-50 text-red-700 font-bold border-red-200'; // Birdie
        if (diff === 0) return 'bg-white text-gray-700 font-medium'; // Par
        if (diff === 1) return 'bg-blue-50 text-blue-800 border-blue-200'; // Bogey
        return 'bg-gray-100 text-gray-800 font-medium border-gray-300'; // Double bogey or worse
    };
    ...
    const renderHalf = (round: ScorecardRound, startHole: number, endHole: number, title: string) => {
        ...
        for (let i = startHole; i <= endHole; i++) {
            ...
            const par = holeData?.par || 4; // fallback
                ...
                <table className="w-full text-sm text-center border-collapse">
                    <thead>
                        <tr className="bg-gray-900 text-white">
                            ...
                            {holes.map(h => (
                                <th key={h.holeId} className="py-2 px-3 border border-gray-700 w-10">{h.holeId}</th>
                            ))}
                    ...
                    <tbody>
                        <tr className="bg-gray-50 text-gray-600 border-b border-gray-200">
                            ...
                            {holes.map(h => (
                                <td key={h.holeId} className="py-2 px-3 border border-gray-200">{h.par}</td>
                            ))}
                        ...
                        <tr>
                            ...
                            {holes.map(h => (
                                <td key={h.holeId} className={`py-2 px-3 border ${getScoreColor(h.score, h.par)}`}>
                                    {h.score || '-'}
                                </td>
                            ))}
                            <td className="py-2 px-3 font-bold border border-gray-200 bg-white">
        ...
        >
            <div className="card w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-200 p-0">
                ...
                <div className="flex items-center justify-between p-5 border-b border-gray-100 bg-gradient-to-r from-green-800 to-green-900 text-white">
                    <div>
                        <h2 className="text-2xl font-black font-serif tracking-tight flex items-center gap-3">
                            {playerName}
                            {isAdmin && (
                                <button
                                    onClick={handleSyncScorecard}
                                    disabled={syncing || loading}
                                    title="Admin Only: Force Sync Player Scorecard from API"
                                    className="p-1.5 bg-green-700/50 hover:bg-green-600 rounded-md transition-colors disabled:opacity-50"
                                >
                                    <RefreshCw size={16} className={syncing ? 'animate-spin' : ''} />
                                </button>
                            )}
                        </h2>
                        <p className="text-green-100/80 font-medium text-sm mt-0.5 uppercase tracking-wider">
                ...
                <div className="p-6 overflow-y-auto flex-1 bg-gray-50/50">
                    {loading ? (
                        ...
                        <div className="text-center py-12 px-4">
                            ...
                            <button
                                onClick={onClose}
                                className="mt-6 px-4 py-2 bg-gray-900 text-white rounded-lg font-medium hover:bg-gray-800 transition-colors"
                            >
                                Default Back
                            </button>
                        </div>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="flex items-center justify-between bg-white p-2 rounded-lg border border-gray-200 shadow-sm">
                                <button
                                    ...
                                    className="p-2 text-gray-500 hover:bg-gray-100 hover:text-gray-900 rounded-md disabled:opacity-30 disabled:hover:bg-transparent transition-colors"
                                >
                                    <ChevronLeft size={20} />
                                </button>
                                ...
                                <div className="flex space-x-1">
                                    {rounds.map((r, idx) => (
                                        <button
                                            ...
                                            className={`px-4 py-1.5 text-sm font-bold rounded-md transition-colors ${idx === activeRoundIndex
                            ...
                            <div className="flex justify-around items-center bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
                                <div className="text-center">
                                    ...
                                    <div className="font-medium text-gray-800">
                                        {activeRound.roundComplete ? 'Round Complete' : `Through Hole ${activeRound.currentHole || '?'}`}
                                    </div>
                                </div>
                                ...
                                <div className="text-center">
                                    ...
                                    <div className="text-base font-bold text-gray-800">
                                        {roundTeeTimes?.[`r${activeRound.roundId}`] || '-'}
                                    </div>
                                </div>
                                <div className="w-px h-10 bg-gray-200"></div>
                                <div className="text-center">
                                    <div className="text-xs text-gray-400 uppercase font-bold tracking-wider mb-1">Today's Score</div>
                                    <div className={`text-2xl font-black ${activeRound.currentRoundScore === 'E' || activeRound.currentRoundScore === '0' ? 'text-gray-700' :
                                        (activeRound.currentRoundScore?.startsWith('-') ? 'text-red-600' : 'text-green-700')
                                        }`}>
                                ...
                                <div className="text-center">
                                    ...
                                    <div className="text-xl font-bold text-gray-800">
                                        {activeRound.totalShots || '-'}
                                    </div>
                                </div>
                            </div>

                            {/* Scorecard Grids (using general panel layout) */}
