...
import { SyncSetting, AutosyncSettings } from '../hooks/useAutosyncSettings';
...
const getNextRunTime = (cron: string, lastRun?: Date): Date | null => {
    ...
    if (cron.startsWith('every ')) {
        ...
        // Use regex to reliably find HH:MM regardless of position
        ...
        // Weekly with day-of-week needs special handling to avoid infinite loops
        ...
        const addInterval = (d: Date): Date => {
            ...
            switch (unit) {
                case 'minute': case 'minutes':
                    r.setMinutes(r.getMinutes() + n, 0, 0); break;
                case 'hour': case 'hours':
                    r.setHours(r.getHours() + n, 0, 0, 0); break;
                case 'day': case 'days':
                    r.setDate(r.getDate() + n);
                    r.setHours(th, tm, 0, 0); break;
...
const SyncSettingCard: React.FC<SyncSettingCardProps> = ({
    ...
    // Sync local state when val.cron changes externally (e.g. simple/advanced toggle)
    useEffect(() => {
        setLocalCron(val.cron);
    }, [val.cron]);
    ...
    const extractTime = (cron: string) =>
        cron.split(' ').find(p => /^\d{1,2}:\d{2}$/.test(p)) || '00:00';

    const extractDayOfWeek = (cron: string) =>
        cron.split(' ').find(p => DAYS_OF_WEEK.includes(p)) || 'Monday';
    ...
    return (
        <div className="card flex flex-col p-4 shadow-sm hover:shadow-md transition-shadow">
                ...
                <div className="flex items-center space-x-4">
                    {onRunAll && (
                        <button
                            ...
                            className={`flex items-center px-3 py-1 text-sm font-medium rounded-md transition-colors ${isRunningAll ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-orange-50 text-orange-600 hover:bg-orange-100 hover:text-orange-700 border border-orange-200'}`}
                        >
                    ...
                    {onRunNow && (
                        <button
                            ...
                            className={`flex items-center px-3 py-1 text-sm font-medium rounded-md transition-colors ${isRunning ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-blue-50 text-blue-600 hover:bg-blue-100 hover:text-blue-700 border border-blue-200'}`}
                        >
            ...
            <div className="flex flex-col space-y-2 bg-gray-50 p-3 rounded-lg border border-gray-100">
                <div className="flex items-center space-x-3">
                    ...
                    {/* Ensure it's using 'every X unit' format for the simple UI, otherwise fallback to text */}
                    {localCron.startsWith('every ') ? (
                        <>
                            <select
                                className="p-2 text-sm border border-gray-300 rounded-md focus:ring-2 focus:ring-green-500 focus:border-green-500 bg-white text-black shadow-sm"
                                ...
                                onChange={(e) => {
                                    ...
                                    let timeSuffix = '';
                                    if (['week', 'weeks'].includes(currentUnit)) {
                                        timeSuffix = ` ${extractDayOfWeek(localCron)} ${existingTime}`;
                                    } else if (['day', 'days', 'month', 'months'].includes(currentUnit)) {
                            ...
                            >
                                {[1, 2, 3, 4, 5, 10, 15, 20, 30, 45].map(num => (
                                    <option key={num} value={num}>{num}</option>
                                ))}
                            </select>

                            <select
                                className="p-2 text-sm border border-gray-300 rounded-md focus:ring-2 focus:ring-green-500 focus:border-green-500 bg-white text-black shadow-sm flex-1"
                                ...
                                onChange={(e) => {
                                    ...
                                    if (['week', 'weeks'].includes(newUnit)) {
                                        // Preserve existing day if already weekly, else default to Monday
                        ...
                        <input
                            ...
                            className="flex-1 p-2 text-sm border border-gray-300 rounded-md focus:ring-2 focus:ring-green-500 focus:border-green-500 bg-white text-black shadow-sm"
                            placeholder="e.g. 0 0 * * 1"
                        />
                    )}

                    {/* Toggle simple/advanced mode — saves immediately to reset to a default */}
                    <button
                        className="text-xs text-green-600 hover:text-green-800 underline ml-2"
                        onClick={() => {
                            if (localCron.startsWith('every ')) {
                                updateEndpoint(settingKey, { cron: '0 0 * * *' });
                            } else {
                                updateEndpoint(settingKey, { cron: 'every 5 minutes' });
                            }
                        }}
                    >
                ...
                {/* Day-of-week + time picker for days/weeks/months in simple mode */}
                {localCron.startsWith('every ') && (() => {
                        ...
                        <div className="flex items-center space-x-3 pl-16 md:pl-24 mt-2 flex-wrap gap-y-2">
                            {isWeekly && (
                                <>
                                    ...
                                    <select
                                        className="p-1.5 text-sm border border-gray-300 rounded-md focus:ring-2 focus:ring-green-500 focus:border-green-500 bg-white text-black shadow-sm"
                ...
                <div className="flex items-center justify-between">
                    ...
                    {latestLog && (
                        <button
                            ...
                            className="flex items-center text-green-600 hover:text-green-800 font-medium cursor-pointer"
                        >
            ...
            {showLogDetails && latestLog && (
                <div className={`mt-2 p-2 rounded text-xs border ${latestLog.status === 'success' ? 'bg-green-50 border-green-200 text-green-800' :
                        latestLog.status === 'error' ? 'bg-red-50 border-red-200 text-red-800' :
